...
function nextlevel(){
    if(isLevelCompleted){
        if(gameLevel<4){
            gameLevel++
            if(gameLevel==4){                
                document.getElementById('gameover').innerHTML='AH ÊÊÊ ! VOCÊ GANHOU O JOGO!';
                isGameOver=true;
            }
    ...
    blockArr.forEach((b)=>{
        if(isBallhitsBlock(ball,b) && b.isShow){
            ...
            if(numObBlocksHit==(numBlockCol-1)*numBlockRow){
               ...
               document.getElementById('gameover').innerHTML='Game Level '+gameLevel+' ESTÁ COMPLETO. CLIQUE EM PROXIMO PARA INICIAR OUTRO NÍVEL';
            }
        }
        if(b.isShow){
            context.beginPath();
...
Ball.prototype.updateBall=function(playerObj){
    ...
    if((this.x+this.radius< playerObj.x || (this.x+this.radius)>(playerObj.x+playerObj.width)) && this.y+this.radius>HEIGHT){
        ...
        if(playerObj.lifeCount<=0){
            document.getElementById('gameover').innerHTML='FIM DO JOGO!!!';
            document.getElementById('liferemaining').innerHTML=playerObj.lifeCount;
            isGameOver=true;
        }
        else{
